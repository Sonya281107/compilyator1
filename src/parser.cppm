...
struct SemTypePool {
    ...
    SemType& get(uint32_t id) {
        return pool[id];
    }
    const SemType& get(uint32_t id) const {
        return pool[id];
    }
    bool valid(uint32_t id) const {
        return id != NO_ID && id < pool.size();
    }
};
...
struct IntLitExpr {
    ...
    SemKind suffix_kind = SemKind::Unknown;  
};
...
struct Program {
    ...
    ExprId add_expr(ExprNode n) {
        ...
        return static_cast<uint32_t>(exprs.size() - 1);
    }
    StmtId add_stmt(StmtNode n) {
        stmts.push_back(std::move(n));
        return static_cast<uint32_t>(stmts.size() - 1);
    }
    DeclId add_decl(DeclNode n) {
        decl_pool.push_back(std::move(n));
        return static_cast<uint32_t>(decl_pool.size() - 1);
    }
    TypeId add_type(TypeNode n) {
        types.push_back(std::move(n));
        return static_cast<uint32_t>(types.size() - 1);
    }

    ExprNode& expr(ExprId id) {
        return exprs[id];
    }
    const ExprNode& expr(ExprId id) const {
        return exprs[id];
    }
    StmtNode& stmt(StmtId id) {
        return stmts[id];
    }
    const StmtNode& stmt(StmtId id) const {
        return stmts[id];
    }
    DeclNode& decl(DeclId id) {
        return decl_pool[id];
    }
    const DeclNode& decl(DeclId id) const {
        return decl_pool[id];
    }
    TypeNode& type(TypeId id) {
        return types[id];
    }
...
bool SemType::is_integer() const {
    return kind == SemKind::I8  || kind == SemKind::I16 || kind == SemKind::I32 || kind == SemKind::I64
        || kind == SemKind::U8  || kind == SemKind::U16 || kind == SemKind::U32 || kind == SemKind::U64;
}

bool SemType::is_unsigned() const {
    return kind == SemKind::U8 || kind == SemKind::U16 || kind == SemKind::U32 || kind == SemKind::U64;
}

bool SemType::is_float() const {
    return kind == SemKind::F32 || kind == SemKind::F64;
}
bool SemType::is_numeric() const {
    return is_integer() || is_float();
}
bool SemType::is_signed_int() const {
    return kind == SemKind::I8 || kind == SemKind::I16 || kind == SemKind::I32 || kind == SemKind::I64;
}
...
bool sem_equal(uint32_t a, uint32_t b, const SemTypePool& pool) {
    ...
    const SemType& sa = pool.get(a);
    const SemType& sb = pool.get(b);
...
static bool is_type_kw(TokenKind k) {
    return k == TokenKind::I8Kw  || k == TokenKind::I16Kw || k == TokenKind::I32Kw
        || k == TokenKind::I64Kw || k == TokenKind::U8Kw  || k == TokenKind::U16Kw
        || k == TokenKind::U32Kw || k == TokenKind::U64Kw || k == TokenKind::F32Kw
        || k == TokenKind::F64Kw || k == TokenKind::BoolKw
...
class ParserImpl {
public:
    ParserImpl(const std::vector<Token>& toks, const std::string& fname, Program& prog)
...
Token ParserImpl::consume() {
    Token t = cur();
    if (!at_end()) ++pos_;
    return t;
}

bool ParserImpl::try_eat(TokenKind k) {
    if (!at(k)) return false;
    consume();
    return true;
}

Token ParserImpl::expect(TokenKind k, const std::string& what) {
    if (!at(k)) error("expected " + what + ", got '" + cur().lexeme + "'");
    return consume();
}

void ParserImpl::error(const std::string& msg) {
    Diagnostic d;
...
InfixInfo ParserImpl::infix(TokenKind k) const {
    switch (k) {
...
ExprId ParserImpl::parse_expr(int min_prec) {
    ...
    while (true) {
        ...
        if (cur().kind == TokenKind::Assign) {
            ...
            AssignExpr node;
            node.span = {expr_span(prog_.expr(left)).begin, expr_span(prog_.expr(rhs)).end};
            node.target = left;
            node.value = rhs;
        ...
        if (cur().kind == TokenKind::AsKw) {
            ...
            consume();
            TypeId tid = parse_type();
            CastExpr node;
            node.span = {lb, type_span(prog_.type(tid)).end};
            node.operand = left;
            node.target = tid;
            left = prog_.add_expr(std::move(node));
            continue;
        ...
        BinaryExpr node;
        node.span = {lb, expr_span(prog_.expr(rhs)).end};
        node.op = op;
        node.left = left;
        node.right = rhs;
        left = prog_.add_expr(std::move(node));
...
ExprId ParserImpl::parse_unary() {
    if (at(TokenKind::Minus) || at(TokenKind::Bang)) {
        ...
        UnaryExpr n;
        n.span = {s, expr_span(prog_.expr(operand)).end};
        n.op = op;
        n.operand = operand;
...
ExprId ParserImpl::parse_postfix(ExprId left) {
    while (true) {
        if (at(TokenKind::LBracket)) {
            SourcePos s = expr_span(prog_.expr(left)).begin;
            consume();
            ExprId idx = parse_expr();
            Token close = expect(TokenKind::RBracket, "']'");
            IndexExpr n;
            n.span = {s, close.span.end};
            n.array = left;
            n.index = idx;
        ...
        if (at(TokenKind::ColonColon)) {
            ...
            if (at(TokenKind::LParen)) {
                ...
                Token close = expect(TokenKind::RParen, "')'");
                n.span = {s, close.span.begin};
                left = prog_.add_expr(std::move(n));
            } else {
                ScopeExpr n;
                n.span = {s, end};
...
std::vector<ExprId> ParserImpl::parse_arg_list() {
    std::vector<ExprId> args;
...
ExprId ParserImpl::parse_primary(bool allow_struct_lit) {
    if (at(TokenKind::IntLiteral)) {
        const std::string& lex = cur().lexeme;

        // A.1.1: detect an optional type suffix (i8 i16 i32 i64 u8 u16 u32 u64).
        struct SufEntry { std::string_view s; SemKind k; };
        static constexpr SufEntry INT_SUFS[] = {
            {"i64", SemKind::I64}, {"i32", SemKind::I32},
            {"i16", SemKind::I16}, {"i8",  SemKind::I8},
            {"u64", SemKind::U64}, {"u32", SemKind::U32},
            {"u16", SemKind::U16}, {"u8",  SemKind::U8},
        };
        SemKind suffix_kind = SemKind::Unknown;
        std::size_t num_end = lex.size();
        for (auto& [s, k] : INT_SUFS) {
            std::size_t slen = s.size();
            if (lex.size() > slen && lex.compare(lex.size() - slen, slen, s) == 0) {
                suffix_kind = k;
                num_end = lex.size() - slen;
                break;
            }
        }

        const char* num_start = lex.data();
        const char* num_stop  = lex.data() + num_end;
        int base = 10;
        if (num_end >= 2 && lex[0] == '0' && (lex[1] == 'x' || lex[1] == 'X')) {
            base = 16;
            num_start += 2;
        } else if (num_end >= 2 && lex[0] == '0' && (lex[1] == 'b' || lex[1] == 'B')) {
            base = 2;
            num_start += 2;
        }
        uint64_t uval = 0;
        std::from_chars(num_start, num_stop, uval, base);

        IntLitExpr n;
        n.span        = cur().span;
        n.value       = static_cast<int64_t>(uval);
        n.suffix_kind = suffix_kind;
        consume();
        return prog_.add_expr(std::move(n));
    }
    ...
    if (at(TokenKind::FloatLiteral)) {
        ...
        struct SufEntry { std::string_view s; SemKind k; };
        static constexpr SufEntry FLT_SUFS[] = {
            {"f64", SemKind::F64}, {"f32", SemKind::F32},
        };
        SemKind suffix_kind = SemKind::Unknown;
        std::size_t num_end = lex.size();
        for (auto& [s, k] : FLT_SUFS) {
            std::size_t slen = s.size();
            if (lex.size() > slen && lex.compare(lex.size() - slen, slen, s) == 0) {
                suffix_kind = k;
                num_end = lex.size() - slen;
        ...
        FloatLitExpr n;
        n.span        = cur().span;
        n.value       = std::stod(std::string(lex, 0, num_end));
        n.suffix_kind = suffix_kind;
        consume();
        return prog_.add_expr(std::move(n));
    }
...
ExprId ParserImpl::parse_cond_expr() {
    return parse_cond_expr_at(0);
}

ExprId ParserImpl::parse_cond_expr_at(int min_prec) {
    ...
    while (true) {
        ...
        if (cur().kind == TokenKind::AsKw) {
            ...
            TypeId tid = parse_type();
            CastExpr n;
            n.span = {lb, type_span(prog_.type(tid)).end};
            n.operand = left;
            n.target = tid;
            left = prog_.add_expr(std::move(n));
            continue;
        ...
        BinaryExpr n;
        n.span = {lb, expr_span(prog_.expr(rhs)).end};
        n.op = op;
        n.left = left;
        n.right = rhs;
        left = prog_.add_expr(std::move(n));
...
ExprId ParserImpl::parse_cond_unary() {
    if (at(TokenKind::Minus) || at(TokenKind::Bang)) {
        SourcePos s = cur().span.begin;
        std::string op = cur().lexeme;
        consume();
...
StmtId ParserImpl::parse_var_decl() {
    SourcePos start = cur().span.begin;
    consume();
...
DeclId ParserImpl::parse_struct_decl() {
    SourcePos start = cur().span.begin;
...
DeclId ParserImpl::parse_type_alias() {
    SourcePos start = cur().span.begin;
...
DeclId ParserImpl::parse_namespace() {
    SourcePos start = cur().span.begin;
    ...
    nd.name = name;
    while (!at(TokenKind::RBrace) && !at_end()) {
        DeclId child = at(TokenKind::FnKw) ? parse_fn_decl(name) : parse_decl();
        nd.decls.push_back(child);
    }
...
ParseResult parse(const std::vector<Token>& tokens, const std::string& filename) {
    ParseResult r;
    ParserImpl parser(tokens, filename, r.program);
    try {
