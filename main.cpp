...
static void dump_tokens(const std::vector<Lexer::Token>& tokens) {
    for (const auto& token : tokens) {
        std::cout << "[" << Lexer::token_kind_name(token.kind) << "] "
                  << "'" << token.lexeme << "' "
...
static void dump_ast(const AST::Program& prog, int indent = 0) {
    ...
    for (AST::DeclId did : prog.top_decls) {
        ...
        if (std::holds_alternative<AST::FnDecl>(d)) {
            const auto& fd = std::get<AST::FnDecl>(d);
            std::cout << pad(indent + 1) << "FnDecl " << fd.name
                      << " [mangled=" << fd.mangled_name << "]\n";

        } else if (std::holds_alternative<AST::StructDecl>(d)) {
            std::cout << pad(indent + 1) << "StructDecl "
                      << std::get<AST::StructDecl>(d).name << "\n";

        } else if (std::holds_alternative<AST::TypeAliasDecl>(d)) {
            std::cout << pad(indent + 1) << "TypeAlias "
                      << std::get<AST::TypeAliasDecl>(d).name << "\n";

        } else if (std::holds_alternative<AST::NamespaceDecl>(d)) {
            std::cout << pad(indent + 1) << "Namespace "
                      << std::get<AST::NamespaceDecl>(d).name << "\n";

        } else if (std::holds_alternative<AST::ImplDecl>(d)) {
            const auto& impl = std::get<AST::ImplDecl>(d);
            std::cout << pad(indent + 1) << "Impl " << impl.struct_name
                      << " (" << impl.methods.size() << " methods)\n";
...
int main(int argc, char* argv[]) {
    ...
    if (output_path.empty()) {
        ...
        auto stem_start = (last_sep == std::string::npos) ? 0 : last_sep + 1;
        auto stem = source_path.substr(stem_start);
        auto dot = stem.rfind('.');
    ...
    if (!sem_result.ok) {
        for (const auto& e : sem_result.errors) {
